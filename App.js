...
import {
   StyleSheet, Text, View , ActivityIndicator, FlatList, Dimensions, 
  Image, Animated, TouchableWithoutFeedback,
  TouchableOpacity, CameraRoll, Share} from 'react-native';

import  {Permissions,  FileSystem}from 'expo-permissions'
...
export default class App extends React.Component {
  ...
  loadWallpapers = () => {
    axios
    .get('https://api.unsplash.com/photos/random?count=60&client_id=eMZE2MqmzWZuCLIKNUNZJ-O4QkkOQkcypdl22nCSJ8o'
    )
  ...
  shareWallpaper =  async image => {
    try {
        await Share.share({
          message:'Checkout this wallpaper  ' + image.url.full
        })
    } catch (error) {
        console.log(error)
    }
